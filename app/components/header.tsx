...
const MobileDrawer = () => {
   ...
   const handleClick = () => {
    ...
    return(
        <>
        <ul className={`sm:hidden z-20 flex flex-col border border-black w-2/4 fixed top-0 right-0 justify-start items-end h-screen bg-black opa ${open ? 'block' : 'hidden'}`} 
        >
        {navdate.map((item) => (
        
               <Link href="#" key={item.id}
               className='flex h-16 items-center justify-start border-black border-b w-full gap-2 pl-4'
               >
                     <Image width={15} height={15} alt={item.text} src={item.icon} />
                     <span className={`capitalize link-${item.id}`} >{item.text}</span>
        ...
        <button ref={button} onClick={handleClick} className='bg-black text-slate-200 p-3 flex flex-col gap-1 rounded-sm absolute right-6 top-6'>
